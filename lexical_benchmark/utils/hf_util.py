...
class CharacterTokenizer(PreTrainedTokenizer):
    def __init__(self, chars: str, model_max_length: int):
        super().__init__()
    ...
    def decode(self, token_ids, skip_special_tokens=True, **kwargs) -> str:
        ...
        if skip_special_tokens:
            tokens = [token for token in tokens if token not in 
                     [self.pad_token, self.eos_token, self.bos_token]]
            
        return self.convert_tokens_to_string(tokens)

    
def load_char_tokenizer(model_max_length=2048,special_token_lst=['\'','|']):
