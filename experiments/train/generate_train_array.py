...
def get_subdirs_by_count(sub_month_dirs:Path,max_num:int)->list:
    """Get the required number of subdirectories"""
    try:
        sorted_sub_dirs = sorted(sub_month_dirs, key=lambda x: int(x.name))
        sub_month_dirs = sorted_sub_dirs[:max_num]
    except ValueError:
        print(f"Warning: Some subdirectory names in {month_dir} are not numeric. Using default sorting.")
        sub_month_dirs = sub_month_dirs[:max_num]
    return sub_month_dirs
