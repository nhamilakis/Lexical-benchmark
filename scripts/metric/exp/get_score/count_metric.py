...
def parseArgs(argv):
    ...
    parser.add_argument('--eval_type_lst', default = ['unprompted_0.3','unprompted_0.6','unprompted_1.5'],
...
def load_data(TextPath:str,sec_frame_path:str,OutputPath,age_range:list,mode:str,eval_type:str):
    ...
    if os.path.exists(TextPath):
        ...
        #sec_frame.to_csv(sec_frame_path)
        '''
        ...
        # output the results
        raw_counts.to_csv(OutputPath+'raw_'+ eval_type + str(age_range[0]) + '_' + str(age_range[1]) + '.csv')
        cum_counts.to_csv(OutputPath+'cum_' + eval_type+ str(age_range[0]) + '_' + str(age_range[1]) +'.csv')
        cum_adjusted_counts.to_csv(OutputPath+'cum_adjusted_' + eval_type + str(age_range[0]) + '_' + str(age_range[1]) +'.csv')
...
def select_words(eval_path, OutputPath,lang,eval_type,age_range):
    ...
    if not eval_type == 'adult_concat' and not eval_type == 'model':
        cum_adjusted_counts = pd.read_csv(OutputPath +'cum_adjusted_'+ eval_type + str(age_range[0]) + '_' + str(age_range[1]) +'.csv', index_col=0)
                                
    elif eval_type == 'adult_concat' :
        cum_adjusted_counts = pd.read_csv(OutputPath +'cum_'+ eval_type + str(age_range[0]) + '_' + str(age_range[1]) +'.csv', index_col=0)
...
lang_lst = ['AE', 'BE']
eval_type_lst = ['unprompted_0.3','unprompted_0.6','unprompted_1.0','unprompted_1.5']
