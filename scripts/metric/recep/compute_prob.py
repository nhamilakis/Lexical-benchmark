...
def parseArgs(argv):
    # Run parameters
    parser = argparse.ArgumentParser(description='Compute pseudo log-probabilities of quantized units with a trained language model.')
    parser.add_argument('--pathQuantizedUnits', type=str, default='test.txt',
    ...
    parser.add_argument('--pathOutputFile', type=str, default='log_prob.txt',
                        help='Path to the output file containing scores.')
    parser.add_argument('--pathLSTMCheckpoint', type=str, default='/data/Lexical-benchmark/recep/phones/50h/00/checkpoint_best.pt',
                        help='Path to the trained fairseq LSTM model.')
    parser.add_argument('--dict', type=str,	default='/data/Lexical-benchmark/recep/phones/50h/00/data-bin/dict.txt',
    ...
    parser.add_argument('--gpu', default=True,
                        help="Run on a cpu or gpu machine.")
...
def main(argv):
    ...
    _ = compute_proba_LSTM(
                        intput_file_seqs, model, task, args.gpu,
