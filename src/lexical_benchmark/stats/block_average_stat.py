...
#TODO: test and merge the results with block_average
@dataclass
class RejectionRate:
...
class RejectionRateResult:
    ...
    def view(self, *, view_type: VIEW_TYPES, avg_type: AVG_TYPES = "average") -> dict[str, t.Any]:
        ...
        if view_type == "type/token":
            return {
                "raw_type/token": self.type_counts.raw/self.token_counts.raw,
                "clean_type/token": self.type_counts.clean/self.token_counts.clean,
                "rejected_type/token": self.type_counts..rejected/self.token_counts..rejected
            }
