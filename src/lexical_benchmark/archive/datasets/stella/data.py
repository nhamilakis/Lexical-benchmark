...
from lexical_benchmark import lb_types, settings, text_lib, utils
...
@dataclass
class StelaModelItem:
    """Handler for a given model directory."""
    ...
    def root_dir(self) -> Path:
        ...
        return self._stela.model_root / self.schema / self.lang / self.month / self.chunk / self.model_type

    @property
    def needs_training(self) -> bool:
        """Check if target file exists."""
        return not (self.root_dir / "training_args.bin").is_file()
...
class STELATranscriptDataset:
    ...
    def __init__(
        ...
        model_root: Path = settings.PATH.model_root,
        gen_root: Path = settings.PATH.gen_root,
    ) -> None:
        self.root_dir = root_dir
        self.model = self._Models(root_dataset=self, model_root=model_root)
        self.generations = self._Generation(root_dataset=self, gen_root=gen_root)
        self.txt = self._TXTSplit(root_dataset=self)
        self.by_month = self._ByMonthSplit(root_dataset=self)
        self.processing = self._Processing(root_dataset=self)
    ...
    class _TXTSplit:
        """Sub-Section managing txt splits."""
        ...
        def __init__(self, root_dataset: "STELATranscriptDataset", txt_root: str = "txt") -> None:
            self._root_dt = root_dataset
            self.root_dir = root_dataset.root_dir / txt_root

        def get_item(self, lang: str, hour: str, section: str) -> TXTTranscriptionItem:
            """Accessor for a single chunk, in the txt structure."""
            return TXTTranscriptionItem(
                lang=lang,
                hour_split=hour,
                section=section,
                _stela=self._root_dt,
            )

        def iter_items(self, lang: str, hour: str | None = None) -> t.Iterable[TXTTranscriptionItem]:
            """Iter on a specific hour split."""
            if hour is None:
                for _hour in self.hour_splits:
                    for section in self.chunks_in_split(lang, _hour):
                        yield self.txt_item(
                            lang=lang,
                            hour=_hour,
                            section=section,
                ...
                for section in self.chunks_in_split(lang, hour):
                    yield self.txt_item(
                        lang=lang,
                        hour=hour,
                        section=section,
    ...
    class _ByMonthSplit:
        """Sub-Section managing by_month splits."""
        ...
        def __init__(self, root_dataset: "STELATranscriptDataset", by_month_root: str = "by_month") -> None:
            self._root_dt = root_dataset
            self.root_dir = root_dataset.root_dir / by_month_root

        def get_item(self, lang: str, month: str, chunk: str) -> ByMonthTranscriptionItem:
    ...
    class _Models:
        """Sub-Section managing models."""

        def __init__(self, root_dataset: "STELATranscriptDataset", model_root: Path = settings.PATH.model_root) -> None:
            self._root_dt = root_dataset
            self.root_dir = model_root / root_dataset.root_dir.name

        def get_item(
            self, lang, month: str, chunk: str, schema: DataSchemaType, model_type: lb_types.ModelType
        ) -> StelaModelItem:
            """Load a model item from given parameters."""
            return StelaModelItem(
                lang=lang,
                month=month,
                chunk=chunk,
                schema=schema,
        ...
        def train_chunks(self, lang: str, split: str, schema_type: DataSchemaType) -> tuple[str, ...]:
            """Extract the list of trainable chunks for given split."""
            match schema:
                case "by_month":
                    current_root = self._root_dt.by_month.root_dir / lang / split
                case "txt":
                    current_root = self._root_dt.txt.root_dir / lang / split
                case _:
                    raise ValueError(f"Bad STELA Schema {schema_type}")

            if current_root.is_dir():
                return [d for d in current_root.iterdir() if d.is_dir() and d.name in settings.TRAIN_CHUNKS]
            raise ValueError(f"Could not locate {current_root} to list available chunks.")

        def iter_models(
            self, lang: str, schema_type: DataSchemaType, model_type: lb_types.ModelType
        ) -> t.Iterable[StelaModelItem]:
            """Iterator over model folders."""
            match schema_type:
                case "by_month":
                    hour_month_items = self._root_dt.by_month.month_splits
                case "txt":
                    hour_month_items = self._root_dt.txt.hour_splits
                case _:
                    raise ValueError(f"Bad STELA Schema {schema_type}")

            for hour_month in hour_month_items:
                for chunk in self.train_chunks(lang=lang, split=hour_month, schema=schema_type):
                    yield self.get_item(
                        lang=lang, month=hour_month, chunk=chunk, schema=schema_type, model_type=model_type
                    )

    class _Generation:
        """Sub-Sectin managing generations."""

        def __init__(self, root_dataset: "STELATranscriptDataset", gen_root: Path = settings.PATH.gen_root) -> None:
            self._root_dt = root_dataset
            self.root_dir = gen_root / root_dataset.root_dir.name

        def get_item():
            """Load Generation item from given parameters."""
            # TODO: not implemented

    class _Processing:
        """Sub-Section managing processing."""

        @property
        def preprocessed_path(self) -> Path:
            """Path to preprocessed version of the dataset."""
            return self._root_dt.root_dir / "src" / "preprocessed"

        @property
        def source_path(self) -> Path:
            """Path to the source dataset."""
            return self._root_dt.root_dir / "src" / "original"

        def __init__(self, root_dataset: "STELATranscriptDataset") -> None:
            self._root_dt = root_dataset

        def build_word_frequencies(self) -> t.Any:
    ...
    def get_item_by_id(
        self, schema_type: DataSchemaType, lang: str, data_id: str
    ) -> TXTTranscriptionItem | ByMonthTranscriptionItem:
    ...
    def iter_items(self, schema: DataSchemaType) -> t.Iterable[TXTTranscriptionItem | ByMonthTranscriptionItem]:
        """Iterator for transcriptions."""
        for lang in self.languages:
            match schema:
                case "by_month":
                    yield from self.by_month.iter_items(lang=lang)
                case "txt":
                    yield from self.txt.iter_items(lang=lang)
                case _:
                    raise ValueError(f"Bad schema {schema}")
