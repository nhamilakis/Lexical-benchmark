...
def custom_format(num: float) -> str:
    """Apply a custom formatting of numbers."""
    if (not np.isnan(num)) and num == int(num):  # Check if the number is effectively an integer
        return f"{int(num)}"
    if 0.001 < abs(num) < 1000:  # Check if the number is between 0 and 0.001
        return f"{num:.3g}"
...
def p_obs_k_ps(k, p, n):
    ...
    Poisson approximation with Stirling approximation for factorial, crashes for k>800; valid only for p small,
    n large, np moderate prob= ((l/k*e)^k) * e^-l / sqrt(2k*pi)   with l=n*p
    """
    lambd = p * n
    prob = np.nan
...
def p_obs_k_list(k, p, n):
    ...
    return {
        "exact": p_obs_k_e(k, p, n),
        "Poisson": p_obs_k_p(k, p, n),
        "PoissonStir": p_obs_k_ps(k, p, n),
        "Gaussian": p_obs_k_g(k, p, n),
        "total": p_obs_k(k, p, n),
    }
...
def accu_model_tok_stats(token_count, ref_corpus_size, gen_corpus_size=None):
    ...
    Frequency stats for a token observed token_count times in the reference corpus (of size ref_corpus_size).
...
def make_accu(ref_count: nlp_tools.TokenCount) -> nlp_tools.TokenCount:
    ...
    accu_count = nlp_tools.TokenCount(dict(zip(accuwords, accucountarray)), name="accu")
...
def generate_ngrams(word: str, n: int = 2) -> list:
...
def build_ngram_model(words: list[str], n: int = 2) -> dict[str, dict[str, float]]:
    ...
    return {
        f"{prefix}": {f"{char}": vals / sum(counts.values()) for char, vals in counts.items()}
        for prefix, counts in model.items()
    }


# sampling one word from the word ngram model
def sample_word(model: dict[str, dict[str, float]], n: int = 3, start_symbol: str = "^", end_symbol: str = "$") -> str:
...
def make_crp(ref_count: pd.DataFrame, alpha: float) -> pd.DataFrame:
    ...
    ngram_model = build_ngram_model(ref_count["word"].to_list(), 3)
    ...
    gen_count = ref_count
    gen_count.columns = pd.Index(["word", "PseudoCount", "freq_m", "correct"])  # only preserve the
    gen_count["count"] = 0

    # set word as index and only preserve the pseudo and true word count
    gen_frame = gen_count[["word", "PseudoCount", "count"]]
    gen_frame = gen_frame.set_index("word")
    ...
    return gen_frame.reset_index()


def get_crp_score(crp_path: Path, cdi_root: str, lang: str, memory_threshold: int) -> dict[int, float]:
