...
def parseArgs(argv):
    ...
    parser.add_argument('--OutputPath', type=str, default = '/scratch2/jliu/STELAWord/productive_vocab/reference/eval/36/unprompted/',
    ...
    parser.add_argument('--temp_lst', default = [0.1,0.3,0.5,0.7,0.9,1],
                        help='a list of temperature parameters for optimization')
    
    parser.add_argument('--beam_lst', default = [1,5,15,20,50],     # to be modified later!!!
                        help='a list of temperature parameters for optimization')
...
def generate_beam(beam_width,temp,length,word_dict,task,generate_model):
    ...
    generated_tokens = []
    generated_words = ''
    seed_text = list(word_dict.keys())[random.randint(0, len(word_dict))]
...
def generate_topk(temp,topk,length,word_dict,task,generate_model):
    ...
    generated_tokens = []
    generated_words = ''
    generated_text = list(word_dict.keys())[random.randint(0, len(word_dict))]
            ...
            for temp in tqdm(temp_lst):
                ...
                while n < prompt.shape[0]:
                    ...
                    try:
                        ...
                        generated = generate_topk(temp,topk,length,word_dict,task,generate_model)    
        ...
        for beam in tqdm(beam_lst):
            for temp in tqdm(temp_lst):
                ...
                while n < prompt.shape[0]:
                    ...
                    try:
                        # generate the prompt; note you need to turn this into the character sequence!
                        generated = generate_beam(beam,temp,length,word_dict,task,generate_model)
